...
export default function AccountModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      <div className="absolute inset-0 bg-slate-950/70 backdrop-blur-sm" onClick={() => !forced && onClose()} />
        ...
        {done ? (
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <Field label={forced ? "Choose a password" : "New password"}>
              <input type="password" value={newPassword} onChange={(e) => setNewPassword(e.target.value)} className="acc-input" placeholder="At least 8 characters" autoComplete="new-password" />
            </Field>
            <Field label="Confirm new password">
              <input type="password" value={confirm} onChange={(e) => setConfirm(e.target.value)} className="acc-input" autoComplete="new-password" />
            </Field>
            <Field label="Current password">
              <input type="password" value={currentPassword} onChange={(e) => setCurrentPassword(e.target.value)} className="acc-input" placeholder="Confirm it's you" autoComplete="current-password" />
            </Field>
            ...
            <div className="flex gap-2 pt-1">
              {!forced && (
                <button type="button" onClick={onClose} className="flex-1 py-2.5 rounded-lg border border-slate-200 text-slate-700 font-medium text-sm hover:bg-slate-50">
                  Cancel
                </button>
              )}
              <button type="submit" disabled={saving} className="flex-1 py-2.5 rounded-lg bg-gradient-to-br from-purple-600 to-pink-600 text-white font-medium text-sm hover:opacity-90 disabled:opacity-50">
